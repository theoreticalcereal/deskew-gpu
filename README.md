...
3. `DESKEW` writes deskewed OME-Zarr volumes under `Top_shear/`.
   When `output_formats` is `ozx`, the workflow publishes zipped OME-Zarr
   archives instead of the folder-form OME-Zarr output.
...
With `--output_formats ozx`, the folder-form `Top_shear/` output is not
published. Instead the workflow publishes:
