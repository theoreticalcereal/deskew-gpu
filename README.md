...
Level `0` top-shear output is stored and labelled in Neuroglancer-oriented
`x, y, z` order. The `x` axis is the output page axis computed from the
original X dimension, `y` is the deskewed/sheared lateral axis, and `z` is the
scaled top-view depth.
